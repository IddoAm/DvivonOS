...
if command -v clang-format &> /dev/null; then
    ...
    for file in $FILES; do
        if ! clang-format --dry-run --Werror --style=file:"$FORMATS_DIR/.clang-format" "$file" &> /dev/null; then
...
if command -v clang-tidy &> /dev/null; then
    ...
    if [ -f "compile_commands.json" ]; then
        ...
        for file in $FILES; do
            if clang-tidy "$file" --config-file="$FORMATS_DIR/.clang-tidy" 2>&1 | grep -q "error:"; then
...
#     for file in $FILES; do
#         if ! python3 "$FORMATS_DIR/check_naming.py" "$file" 2>&1 | grep -q "All naming conventions OK"; then
#             python3 "$FORMATS_DIR/check_naming.py" "$file" 2>&1 | grep -v "All naming conventions OK" || true
...
if [ $ERRORS -eq 0 ]; then
    ...
    echo ""
    echo "Quick fixes:"
    echo "  - Format code: find src -name '*.c' -o -name '*.h' | xargs clang-format -i --style=file:scripts/formats/.clang-format"
    echo "  - Check naming: python3 scripts/formats/check_naming.py <file>"
    exit 1
fi
